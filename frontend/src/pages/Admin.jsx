...
// const socket = io("http://localhost:3000"); // same as user side
...
//             socket.emit("join", { userId: "admin-id", role: "admin" });

//             if (selectedUser?.chatID) {
//                 socket.emit("join-room", selectedUser.chatID);
//             }
//         });
...
const Admin = () => {
    ...
    useEffect(() => {
        ...
        socket.on("connect", () => {
            console.log("Admin connected:", socket.id);
            // Join as admin with proper identification
   ...
   socket.on("receive-message", (msg) => {
    ...
    // Handle room changes when user selection changes
    useEffect(() => {
        const socket = socketRef.current;
        if (!socket) return;
        ...
        // Join new room if user is selected
        if (selectedUser?.chatID) {
            socket.emit("join-room", selectedUser.chatID);
            currentRoomRef.current = selectedUser.chatID;
            console.log("Joined room:", selectedUser.chatID);
        }
    }, [selectedUser]);
    ...
    const handleSendMessage = async () => {
        ...
        try {
            ...
            await axios.post(`${import.meta.env.VITE_BACK_END_URL}/chat/message`, {
                chatID: selectedUser.chatID,
                message: newMessage,
                senderType: "admin",
                userID: selectedUser._id
